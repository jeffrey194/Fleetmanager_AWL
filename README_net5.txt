...
using the data type HelloWorld from hello_world.idl.

This example builds one application (hello_worldProgram.cs)
which can run a publisher example (hello_worldPublisher.cs)
or a subscriber example (hello_worldSubscriber.cs).
...
To modify the data being sent: edit the hello_worldPublisher.cs
file where it says
...
Open hello_world.csproj with Visual Studio to edit, compile, debug,
and run the project.
...
2) In hello_worldProgram.cs remove the code that uses System.CommandLine.
